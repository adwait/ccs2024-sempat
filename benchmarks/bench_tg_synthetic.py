...
# Microarchitectural platform depth (Figure 5)
PDEPTHS = [2, 3, 4, 5, 6, 7, 8]
# Grammar depth in excess of platform depth (true grammar is gdepth_+pdepth)
GDEPTHS_ = [0, 1, 2]

configwise_times = {}

def get_tg_config (benchname, pdepth, gdepth):
    return f"{benchname}_pdepth{pdepth}_gdepth{gdepth}"

# Generate Patterns for the PlatSynth synthetic platform (Figure 5)
def synthetic_tg_command (benchname, pdepth, gdepth):
    config = get_tg_config(benchname, pdepth, gdepth)
    command = f"secant --verbosity 3 -s {config} -c syntab -p synthetic -q pipe_depth={pdepth} -a synthetic:tg -b depth={gdepth}"
...
def main(args):
    ...
    all_times = [["Microarch. platform\ndepth (pdepth)", "Grammar depth =\npdepth + 0", \
                    "Grammar depth =\npdepth + 1", "Grammar depth =\npdepth + 2"]]

    # Depth based generation
    for pdepth in PDEPTHS:
        times = [pdepth]
        for gdepth_ in GDEPTHS_:
            gdepth = pdepth + gdepth_
            
            (conf, com) = synthetic_tg_command(benchname, pdepth, gdepth)
    ...
    with open(f"{benchname}/all_times.md", "w") as f:
        f.write("### Synthetic TG Benchmark (Figure 12(d))\n\n")
