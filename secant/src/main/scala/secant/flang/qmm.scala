...
// qmm.scala:
// Taken from https://github.com/jjfiv/qmm-scala/blob/master/main.scala
package qmm
...
object qmm {
    ...
    def method(minterms: List[Int], dontcares: List[Int], vars: List[String]) : List[List[(String, Boolean)]] = {
...
case class PITable(val rows: List[PrimeImplicant], val cols: Set[Int], val results: Set[Implicant], val vars: List[String]) {
    ...
    def toSumOfProducts(vars: List[String]) : List[List[(String, Boolean)]] = {
        assert(finished)

        results.map(_.withVars(vars)).toList
    }
...
class Implicant(val minterm: Int, val tag:Int=1, val group:List[Int]=Nil) {
    ...
    def withVars(vars: List[String]): List[(String, Boolean)] = {
